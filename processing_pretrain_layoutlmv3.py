...
class LayoutLMv3PretrainProcessor(ProcessorMixin):
    r"""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        ### ml: add mi tokenizer application codes
        ### ml: using pretrained dalle image tokenizer as 1st try
        ### input shape: (3, 112, 112) output shape: (196)
        preprocessed_pixel_values = self.visual_token_transform(torch.tensor(images))

        ### ml: apply image tokenizer and use its result as label
        with torch.no_grad():
            z_logits = self.image_tokenizer(preprocessed_pixel_values)
            im_labels = torch.argmax(z_logits, axis=1).flatten(1)

        # im_prediction_scores = self.im_head(sequence_output)
        encoded_inputs['im_labels'] = im_labels
